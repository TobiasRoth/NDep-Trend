...
res <- tibble(Measures = c("Alpha-diversity", "Beta-diversity", "Gamma-Diversity", "Temperature value", "Huminity value", "Nutrients value", "Light value"),
              'Period 1' = NA,
              'Period 2' = NA,
              'Period 3' = NA,
              'Temporal-Trend' = NA,
...
tmp <- stan_glmer(SR ~ yr + (yr|aID_STAO), data = surv, family = poisson,
                  cores = ncores, iter = niter)
res[1, "Temporal-Trend"] <- fixef(tmp)["yr"]
res[1, c('2.5%', '97.5%')] <- posterior_interval(tmp, pars = "yr")
