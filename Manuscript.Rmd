...
The average $\pm$ SD mean annual temperature at our sites was `r mean(sites$TemperatureR / 10)` $\pm$ `r sd(sites$TemperatureR / 10)` $^\circ$C. 
 ...
 Further, we estimated atmospheric N deposition for each site using a pragmatic approach  that combines monitoring data, spatial interpolation methods, emission inventories, statistical dispersion models and inferential deposition models [@RihmAchermann2016].
...
Finally, we used inclination as proxy for land-use intensity because we expected that steeper sites are likely to be less intensively managed [@Strebel2015]. The average inclination at our sites was `r mean(sites$InclinationR)` $\pm$ `r sd(sites$InclinationR)`$^\circ$. 
...
```{r Ndepchange, fig.cap="(a) The nitrogen (N) deposition in 2000 and (b) the change in N deposition between 2000 and 2015 along the N deposition gradient of the study sites in 2000."}
...
Species communities are shaped by a range of factors, including deterministic processes such as environmental filtering or competitive interactions [@Goetzenberger2012; @Janevcek2013]. Such factors select for species with specific characteristics. Community assembly theories thus suggest that the factors that drive the composition of species in a community can be inferred from comparing the characteristics of the species in the community with random expectation [@Chase2011]. We adopted this idea and compared the indicator values of species that disappeared or colonized a site with the indicator values from randomly chosen species from the same site (Appendix A). We found that the nutrient values of colonizing species showed largest deviations from random expectation (Table  \@ref(tab:difffromrandomtab)) suggesting that N deposition or other factors that change the nutrient content of soils were the most important drivers of the change in species composition over the last 15 years in Swiss mountain hay meadows.
