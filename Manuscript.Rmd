...
getsamples <- function(s, nspec, d) {
  sample(d$T, nspec, replace = FALSE) %>% mean(na.rm = TRUE)
}
...
getsim <- function(x) {
  tt <- pl %>% filter(aID_STAO == x) %>% 
    ...
    summarise(Occ1 = sum(Visit == 1),
              Occ2 = sum(Visit == 2),
              Occ3 = sum(Visit == 3))
  gain <- (tt$Occ1 == 0 & tt$Occ2 == 1) | (tt$Occ2 == 0 & tt$Occ3 == 1)
  lost <- (tt$Occ1 == 1 & tt$Occ2 == 0) | (tt$Occ2 == 1 & tt$Occ3 == 0)
  tmpgain <- map_dbl(1:nsim, getsamples, nspec = sum(gain), d = tt)
  tmplost <- map_dbl(1:nsim, getsamples, nspec = sum(lost), d = tt)
  list(
    gain = mean(tt$T[gain], na.rm = TRUE),
    gainran = mean(tmpgain, na.rm = TRUE),
    gainransd = sd(tmpgain, na.rm = TRUE),
    lost = mean(tt$T[lost], na.rm = TRUE),
    lostran = mean(tmplost, na.rm = TRUE),
    lostransd = sd(tmplost, na.rm = TRUE))
}

tt <- map(sites$aID_STAO, getsim)

sites$ngain <-  tt %>% map_dbl(function(x) (x$gain - x$gainran))
sites$nlost <-  tt %>% map_dbl(function(x) (x$lost - x$lostran))
