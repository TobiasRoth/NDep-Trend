...
Another key driver for plants that is likely to be affected by climate change is precipitation [@Beier2012]. We used the annual precipitation per site from the WordClim database [@Fick2017]. 
The average annual precipitation at our sites was `r mean(sites$PrecipitationR)` $\pm$ `r sd(sites$PrecipitationR)` mm. Further, we estimated atmospheric N deposition for each site using a pragmatic approach  that combines monitoring data, spatial interpolation methods, emission inventories, statistical dispersion models and inferential deposition models [@RihmAchermann2016].
